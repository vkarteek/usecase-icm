...
const app = express();   // ✅ MUST BE BEFORE using app.use()
...
//       const toolList = await getMcpTools();
//       console.log("\nAVAILABLE TOOLS:", toolList.tools.map(t => t.name));
...
//       // Step 1 → Ask LLM with tool definitions
//       console.log("\nSTEP 1 → Sending user message + tools to OpenAI");
...
//       // If no tool selected → direct answer from LLM
//       if (!assistant.tool_calls) {
//         console.log("\nLLM returned direct answer (no tool needed)");
//         return res.json({ answer: assistant.content });
//       }
  
//       const toolCall = assistant.tool_calls[0];
//       console.log("\nSTEP 2 → LLM selected tool:", toolCall.function.name);
  
//       const toolName = toolCall.function.name;
//       const args = JSON.parse(toolCall.function.arguments);
  
//       //  Step 3 → Execute tool on your MCP server
//       console.log("STEP 3 → Running MCP tool with args:", args);
  
//       const toolResult = await callMcpTool(toolName, args);
//       console.log("TOOL RESULT:", toolResult);
  
//       //  Step 4 → Send tool result back to LLM
//       console.log("\nSTEP 4 → Sending tool result back to OpenAI for final reasoning");
  
//       const final = await openai.chat.completions.create({
//         model: "gpt-4o",
//         messages: [
//           { role: "user", content: userMessage },
//           assistant, // must contain tool_calls
//           {
//             role: "tool",
//             tool_call_id: toolCall.id,
...
//       console.log("\nSTEP 5 → FINAL LLM ANSWER:", finalAnswer);
  
//       //  Step 5 → Return final natural language answer to browser
//       res.json({ answer: finalAnswer });
